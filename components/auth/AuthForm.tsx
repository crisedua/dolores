    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            const { error } = await supabase.auth.signInWithOAuth({
                provider: 'google',
                options: {
                    redirectTo: `${window.location.origin}/auth/callback`, // Supabase will handle the redirect, ensuring we get back to the app
                },
            });
            if (error) throw error;
        } catch (err: any) {
        ...
        <div className="w-full max-w-md p-8 bg-[#0F0F0F] border border-[#222] rounded-2xl shadow-2xl">
            ...
            <div className="mt-8 border-t border-[#222] pt-6">
                ...
                <button
                    ...
                    disabled={loading}
                    className="w-full bg-[#1A1A1A] text-white font-medium py-3 rounded-xl border border-[#333] hover:bg-[#222] transition-colors flex items-center justify-center gap-2 group"
                >
